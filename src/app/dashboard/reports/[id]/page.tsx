...
export default function ReportDetailPage({ params }: { params: { id: string } }) {
  ...
  const getStatusCfg = (status: string): { label: string; style: React.CSSProperties } => {
    const map: Record<string, { label: string; style: React.CSSProperties }> = {
      PENDING:     { label: 'В обработка', style: { background: 'rgba(255,167,38,0.12)', color: '#FFA726', borderColor: 'rgba(255,167,38,0.3)' } },
      IN_REVIEW:   { label: 'Преглеждан',   style: { background: 'rgba(99,179,237,0.12)', color: '#63B3ED', borderColor: 'rgba(99,179,237,0.3)' } },
      IN_PROGRESS: { label: 'В процес', style: { background: 'rgba(139,92,246,0.12)', color: '#A78BFA', borderColor: 'rgba(139,92,246,0.3)' } },
      RESOLVED:    { label: 'Решен',    style: { background: 'rgba(6,214,160,0.12)',  color: '#06D6A0', borderColor: 'rgba(6,214,160,0.3)' } },
      REJECTED:    { label: 'Отхвърлен',    style: { background: 'rgba(255,71,87,0.12)',  color: '#FF4757', borderColor: 'rgba(255,71,87,0.3)' } },
    };
  ...
  if (!report) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: 'var(--s-bg)' }}>
        <p className="text-[var(--s-muted)]">Сигналът не е намерен</p>
      </div>
    );
  }
      ...
      <div className="relative overflow-hidden py-12 px-6 border-b border-[var(--s-border)]">
        ...
        <div className="max-w-4xl mx-auto relative">
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-xs uppercase tracking-[0.4em] text-[var(--s-muted)]">Детайли на сигнал</p>
      ...
      <div className="max-w-4xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="site-card rounded-2xl p-6">
          <div className="grid md:grid-cols-2 gap-4 mb-6 text-sm">
            {[
              { label: 'Категория', value: formatCategoryLabel(report.category || report.categoryId, 'Без категория') },
              { label: 'Приоритет', value: report.priority },
              { label: 'Създаден', value: new Date(report.createdAt).toLocaleString('bg-BG') },
              { label: 'Координати', value: `${report.latitude.toFixed(4)}, ${report.longitude.toFixed(4)}` },
            ].map(({ label, value }) => (
              <div key={label}>
          ...
          {report.address && (
            <div className="mb-4 text-sm">
              <span className="text-xs uppercase tracking-[0.3em] text-[var(--s-muted)]">Адрес</span>
              <p className="text-[var(--s-text)] mt-1">{report.address}</p>
            </div>
          )}
          <div>
            <span className="text-xs uppercase tracking-[0.3em] text-[var(--s-muted)]">Описание</span>
        ...
        {/* History */}
        <div className="site-card rounded-2xl p-6">
          <h2 className="rc-display font-bold text-lg text-[var(--s-text)] mb-4">История</h2>
        ...
        <div className="site-card rounded-2xl p-6">
          <h2 className="rc-display font-bold text-lg text-[var(--s-text)] mb-4">
            Коментари ({report.comments.length})
          </h2>
            ...
            {report.comments.map((c: any) => (
              <div key={c.id} className="rounded-2xl border border-[var(--s-border)] p-4 bg-[var(--s-surface2)]">
                <div className="flex items-center justify-between mb-2 text-xs text-[var(--s-muted)] uppercase tracking-[0.25em]">
                  ...
                  <span>{new Date(c.createdAt).toLocaleString('bg-BG')}</span>
                </div>
