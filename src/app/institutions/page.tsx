      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between gap-4 mb-8">
          <div>
            ...
            <h1 className="rc-display text-3xl font-bold text-[var(--s-text)] mt-2">Сигнали към вашата институция</h1>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={load} className="btn-site-ghost text-sm px-4 py-2 rounded-xl">Обнови</button>
            <button onClick={logout} className="btn-site-primary text-sm px-4 py-2 rounded-xl">Изход</button>
          </div>
        </div>
        ...
        {loading ? (
          <div className="rounded-3xl site-card-glass p-8 text-[var(--s-muted)]">Зареждане...</div>
        ) : reports.length === 0 ? (
          <div className="rounded-3xl site-card-glass p-8 text-[var(--s-muted)]">Няма сигнали към вашата институция.</div>
        ) : (
                ...
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="text-xs text-[var(--s-muted)] mt-1">
                      {report.category?.nameBg || 'Без категория'}{report.district ? ` • ${report.district}` : ''}
                    </p>
                  </div>
