...
async function syncTaxonomyData() {
  ...
  if (!hasTaxonomyTables) {
    console.warn(
      '⚠️ Taxonomy i18n models are not available in Prisma Client. Run `npx prisma generate` and apply migrations, then rerun seed/sync.'
    );
  }
  ...
  for (let index = 0; index < taxonomy.categories.length; index += 1) {
    ...
    console.log(`➡️ [${index + 1}/${totalCategories}] Processing category: ${category.name}`);
