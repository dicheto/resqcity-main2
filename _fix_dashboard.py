...
import { formatCategoryLabel } from '@/lib/report-format';
    ...
    <div className="min-h-screen" style={{ background: 'var(--s-bg)' }}>
      ...
      <div className="max-w-6xl mx-auto px-6 py-10 space-y-8">
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          {[
            { label: 'Сигнала', value: total, icon: '📋', color: 'var(--s-violet)', glow: 'var(--s-glow-v)' },
            { label: 'В обработка', value: pending, icon: '⏳', color: '#FFA726', glow: 'rgba(255,167,38,0.2)' },
            { label: 'Решени', value: resolved, icon: '✅', color: 'var(--s-teal)', glow: 'var(--s-glow-t)' },
          ].map(({ label, value, icon, color, glow }) => (
            <div key={label} className="site-card p-6 rounded-2xl hover:cursor-default">
              <div className="flex items-center justify-between mb-4">
                <span className="text-2xl">{icon}</span>
                <span className="text-[10px] uppercase tracking-[0.4em] text-[var(--s-muted)]">{label}</span>
              </div>
              <p className="rc-display font-bold text-4xl" style={{ color }}>{value}</p>
              <div className="mt-3 h-1 rounded-full opacity-30" style={{ background: color }} />
            </div>
          ))}
        </div>
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          ...
          <Link href="/report-incident" className="site-card p-5 rounded-2xl block group"
...
with open(r'c:\Users\deo_d\Downloads\file\ResQCity\src\app\dashboard\page.tsx', 'w', encoding='utf-8') as f:
    f.write(content)
